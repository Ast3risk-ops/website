+++
title = '/uses'
description = 'What I use'
date = 2025-01-11T11:08:41-05:00
draft = false
showComments = false
+++

## Hardware

- {{< icon "framework" >}} Framework Laptop 16
  - {{< icon "microchip" >}} AMD Ryzen 7 7840HS (16) @ 3.80 GHz
  - {{< icon "memory" >}} 64 GB
  - {{< icon "gpu" >}} AMD Radeon RX 7700S [Discrete] & AMD Radeon 780M Graphics [Integrated]
  - {{< icon "computer" >}} Windows 11 IoT Enterprise LTSC with hardening (for now)
  <!-- {{< icon "desktop" >}} Explorer -->

## Software

- [Trivalent](https://github.com/secureblue/Trivalent) on Linux, Edge with [topaz's policies](https://github.com/topaz8/windows-edge-policies) on Windows.
- [Obsidian](https://obsidian.md) (notes)
- [Fooyin](https://www.fooyin.org/) & [Jellyfin](https://jellyfin.org) (music, yes I keep all my music locally)
- [Zed](https://zed.dev/) with AI features disabled

## Frameworks

- {{< icon "python" >}} Python
- {{< icon "materialformkdocs" >}} Material for MkDocs/Zensical
- {{< icon "hugo" >}} Hugo
- {{< icon "nuxt" >}} Nuxt
